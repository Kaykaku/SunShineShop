...
<head>
<meta charset="utf-8">
<title>Sunshine Shop - Page ${page} - All News</title>
...
<link rel="apple-touch-icon-precomposed"
	href="images/apple-touch/57.png">
			...
			<div class="row">
				...
				<div class="span12">
					<div class="title-area">
						<h1 class="inline">
							<span class="light">Sunshine</span> News
						</h1>
						<h2 class="inline tagline">- Where smart people read dumb
							things</h2>
					</div>
				</div>
				...
				<section class="span8 blog">
						...
						<article class="post format-standard sticky">
							<div class="post-inner">
								<div class="post-title">
									<h2>
										<a href="#"><span class="light">News</span> ${item.title }</a>
									</h2>
									<div class="metadata">
										${item.createDate } <a href="#">No comments</a> / Posted in: <a
											rel="category tag" title="View all posts in aciform" href="#">Sunshine</a>
									</div>
								</div>
								<p class="push-down-25">${item.description }</p>
								<a href="<%=Common.url %>/news-details?id=${item.newId}"
									class="btn btn-primary bold higher">CONTINUE READING</a>
							</div>
						</article>
					</c:forEach>
					...
					<div class="pagination">
						<ul>
							<li><a href="<%=Common.url %>/news?page=${page!=1?page-1:1}"
								class="btn btn-primary"><span class="icon-chevron-left"></span></a></li>
							<c:forEach begin="1" end="${maxPage}" varStatus="loop">
								<li class="${(loop.index)==page?'active':''}"><a
									href="<%=Common.url %>/news?page=${loop.index}">${loop.index}</a></li>
							</c:forEach>
							<li><a
								href="<%=Common.url %>/news?page=${page!=maxPage?page+1:maxPage}"
